...
type DetailInput = { amount: string; date: string; referenceNo?: string | null | undefined; note?: string | null | undefined; invoiceNo?: string | null | undefined };
...
export type ReceivablesMoneyOperation =
  ...
  | { type: "invoice.void"; ledgerId: string; detailId: string; input: { revision: number; reason: string } }
  ...
  | { type: "receipt.void"; ledgerId: string; detailId: string; input: { revision: number; reason: string } }
  | { type: "writeoff.patch"; ledgerId: string; input: { revision: number; reason: string; writeoffAmount: string } };
...
function snapshot(row: Ledger): Prisma.InputJsonObject { return JSON.parse(JSON.stringify(row)) as Prisma.InputJsonObject; }
...
async function updateParent(tx: Tx, context: ReceivablesMoneyContext, access: ReceivablesAccess, before: Ledger, reason: string, action: string, beforeAnomaly: string | null, nextWriteoff = before.writeoffAmount) {
  ...
  const changed = await tx.receivableLedger.updateMany({ where: { id: before.id, revision: before.revision, status: "active" }, data: { openingChargeDate: latestInvoice?.invoiceDate ?? null, writeoffAmount: nextWriteoff, updatedBy: context.principal.accountId, revision: { increment: 1 } } });
  ...
  await writeCriticalAudit(tx, { actorId: context.principal.accountId, action, objectType: "receivable_ledger", objectId: before.id, requestId: context.requestId, ...auditScope(access, before.financeDepartmentId), reason, metadata: { before: snapshot(before), after: { ...snapshot(after), amounts: afterAmounts } } });
  return { ledger: { ...after, ...afterAmounts, writeoffAmount: after.writeoffAmount.toFixed(4) }, amounts: afterAmounts };
}
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (invoice) {
      if (creating) {
        ...
        created = await tx.receivableInvoice.create({ data: { ledgerId: before.id, invoiceDate: date(input.date), invoiceNo: text(input.invoiceNo) ?? null, amount: decimal(input.amount, amountInvalid, true), note: text(input.note) ?? null, createdBy: context.principal.accountId } });
      } else {
      ...
      const input = operation.input as DetailInput;
      created = await tx.receivableReceipt.create({ data: { ledgerId: before.id, receiptDate: date(input.date), referenceNo: text(input.referenceNo) ?? null, amount: decimal(input.amount, amountInvalid, true), note: text(input.note) ?? null, createdBy: context.principal.accountId } });
    } else {
      const input = operation.input as UpdateDetailInput;
    ...
    await updateParent(tx, context, access, before, reason, `receivables.money.${operation.type}`, beforeAnomaly);
    if (creating) return responseDetail(created as { amount: Prisma.Decimal });
    return invoice ? responseDetail(await tx.receivableInvoice.findUniqueOrThrow({ where: { id: detailId! } })) : responseDetail(await tx.receivableReceipt.findUniqueOrThrow({ where: { id: detailId! } }));
...
async function writeoffCommand(context: ReceivablesMoneyContext, operation: Extract<ReceivablesMoneyOperation, { type: "writeoff.patch" }>) {
  return prisma.$transaction(async (tx) => {
    const access = await lockAuthority(tx, context.principal);
    const before = await writableLedger(tx, access, operation.ledgerId);
    if (before.revision !== operation.input.revision) throw conflict();
...
export async function writeReceivablesMoney(context: ReceivablesMoneyContext, operation: ReceivablesMoneyOperation) {
  return operation.type === "writeoff.patch" ? writeoffCommand(context, operation) : detailCommand(context, operation);
}
