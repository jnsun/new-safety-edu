...
Page({
  data: { busy: false, wechatPhoneBusy: false, sending: false, showSmsFallback: false, profileRequired: false, maskedPhone: '', wechatPhoneVerificationToken: '', error: '', phone: '', code: '', name: '', reason: '', purpose: 'wechat_bind', departments: [], departmentIndex: -1, countdown: 0 },
  async onLoad() {
  ...
  toggleSmsFallback() { this.setData({ showSmsFallback: !this.data.showSmsFallback, profileRequired: false, error: '' }) },
  identityDetails() {
  ...
  async verifyWechatPhone(e) {
    ...
    try {
      const data = await api.request('/api/wechat/identity/wechat-phone', 'POST', { code: e.detail.code })
      if (data.status === 'bound') return this.finish(data)
      this.setData({ profileRequired: true, showSmsFallback: false, maskedPhone: data.maskedPhone, wechatPhoneVerificationToken: data.wechatPhoneVerificationToken })
    }
    catch (error) { this.setData({ error: error.message }) }
    finally { this.setData({ wechatPhoneBusy: false }) }
  },
  async submitWechatProfile() {
    const details = this.identityDetails()
    if (!details) return wx.showToast({ title: '请填写姓名并选择部门', icon: 'none' })
    this.setData({ busy: true, error: '' })
    try { this.finish(await api.request('/api/wechat/identity/confirm', 'POST', { ...details, wechatPhoneVerificationToken: this.data.wechatPhoneVerificationToken })) }
    catch (error) { this.setData({ error: error.message }) }
