...
function BlockPreview({ block }: { block: CoursewareBlock }) {
  ...
  if (block.type === "checkpoint") return <section><span className="preview-label">随堂题</span><h4>{block.prompt || "题目会显示在这里"}</h4><div className="preview-options">{block.options.map((option, index) => <button type="button" className={selectedAnswers.includes(index) ? "selected" : ""} key={index} onClick={() => setSelectedAnswers(block.questionType === "multiple_choice" ? selectedAnswers.includes(index) ? selectedAnswers.filter((value) => value !== index) : [...selectedAnswers, index] : [index])}>{String.fromCharCode(65 + index)}. {option || "待填写"}</button>)}</div>{selectedAnswers.length > 0 && block.explanation && <p className="preview-explanation">解析：{block.explanation}</p>}</section>;
  if (block.type === "scenario") { const choice = selectedChoice === null ? null : block.choices[selectedChoice]; return <section><span className="preview-label">情境选择</span><h4>{block.prompt || "情境描述会显示在这里"}</h4><div className="preview-options">{block.choices.map((item, index) => <button type="button" className={selectedChoice === index ? "selected" : ""} key={index} onClick={() => setSelectedChoice(index)}>{item.label || `选择 ${index + 1}`}</button>)}</div>{choice && <div className="preview-choice-result"><p><strong>可能后果：</strong>{choice.consequence || "待填写"}</p><p><strong>制度依据：</strong>{choice.basis || "待填写"}</p></div>}</section>; }
