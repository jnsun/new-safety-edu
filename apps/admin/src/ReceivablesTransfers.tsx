...
function ReceivablesExports({ accountId, scopeFingerprint, access }: { accountId: string; scopeFingerprint: string; access: ReceivablesAccess }) {
  ...
  return <div className="receivables-page">
    ...
    <Card className="filters" title="导出筛选"><Form layout="vertical"><Space wrap align="start"><Form.Item label="基础记录状态"><Select<NonNullable<ReceivablesFilters["status"]>> value={filters.status ?? "active"} style={{ width: 150 }} options={[{ value: "active", label: "有效" }, { value: "voided", label: "已作废" }, { value: "all", label: "全部" }]} onChange={(status) => { setFilters((current) => ({ ...current, status })); changeSelection(); }} /></Form.Item><Form.Item label="合同 / 项目 / 客户"><Input allowClear value={filters.search ?? ""} onChange={(event) => { setFilters((current) => ({ ...current, search: event.target.value })); changeSelection(); }} /></Form.Item></Space><Space wrap align="start">{currentPreview && receivablesExportCategoryIds.map((category) => <Form.Item key={category} label={exportCategoryLabels[category]}><Select mode="multiple" allowClear value={categoryFilters[category] ?? []} style={{ minWidth: 190 }} options={currentPreview.categoryOptions[category]} onChange={(values) => { setCategoryFilters((current) => ({ ...current, [category]: values })); changeSelection(); }} /></Form.Item>)}</Space>{preview.isError && <Alert type="error" showIcon message="导出预览失败" description={(preview.error as Error).message} />}{currentPreview && <><Alert type={currentPreview.rowCount ? "info" : "warning"} showIcon message={`匹配 ${currentPreview.rowCount} 条记录`} /><Form.Item label="导出字段（默认选择有内容的字段）"><Checkbox.Group value={columns} onChange={(values) => { setColumns(values as string[]); setColumnsInitialized(true); setIdempotencyKey(crypto.randomUUID()); }} options={currentPreview.columns.map((column) => ({ value: column.id, label: `${column.label}（${column.nonEmptyCount}）` }))} /></Form.Item><Button type="primary" loading={create.isPending} disabled={!currentPreview.rowCount || !columns.length} onClick={() => create.mutate()}>创建导出任务</Button></>}</Form></Card>
