...
export function QuestionsPage() {
  ...
  return <div className="admin-workspace training-production-page">
    <AdminPageHeader title="题库与试卷" description="先维护题库与题目，再按固定或随机方式组卷；日常挑战复用已启用题目。" actions={<><Button icon={<PlusOutlined />} onClick={() => { setEditingBank(undefined); setBankOpen(true); }}>新建题库</Button><Button icon={<PlusOutlined />} disabled={!selectedBankId} onClick={() => setQuestionOpen(true)}>新增题目</Button><Button icon={<UploadOutlined />} onClick={() => { setImportResult(undefined); setImportOpen(true); }}>批量导入</Button><Button type="primary" onClick={() => { setEditingPaper(undefined); setPaperMode("fixed"); setPaperBankId(selectedBankId); setPaperOpen(true); }}>新建试卷</Button></>} />
    {(banks.isError || questions.isError || papers.isError) && <Alert className="admin-conflict" type="error" showIcon message="题库内容加载失败" description="请检查网络后重新加载；请求失败时不会显示为暂无数据。" action={<Button onClick={() => void Promise.all([banks.refetch(), questions.refetch(), papers.refetch()])}>重新加载</Button>} />}
    <Tabs items={[
