...
export function ReceivablesAdmin({ accountId, scopeFingerprint, access, section }: { accountId: string; scopeFingerprint: string; access: ReceivablesAccess; section: Section }) {
  ...
  const save = useMutation({
    mutationFn: async (values: Record<string, unknown>) => {
      if (section === "grants") {
        ...
        const normalized = normalizeReceivablesGrantDraft(role, { canCreate: !!values.canCreate, canExport: !!values.canExport, canViewAll: !!values.canViewAll, departments: (values.departmentScopes as Array<{ departmentId: string; canRead: boolean; canWrite: boolean }> | undefined) ?? [] });
  ...
  const changeGrantRole = (role: ReceivablesGrant["role"]) => {
    ...
    const normalized = normalizeReceivablesGrantDraft(role, {
      canCreate: !!form.getFieldValue("canCreate"), canExport: !!form.getFieldValue("canExport"), canViewAll: !!form.getFieldValue("canViewAll"),
  ...
  const openEditor = (row?: ReceivablesGrant | ReceivablesDepartment | ReceivablesDictionaryOption) => {
    ...
    else if (section === "grants") { const grant = row as ReceivablesGrant; const normalized = normalizeReceivablesGrantDraft(grant.role, { canCreate: grant.canCreate, canExport: grant.canExport, canViewAll: grant.canViewAll, departments: emptyScopes.map((scope) => { const existing = grant.departments.find((item) => item.financeDepartmentId === scope.departmentId); return { ...scope, canRead: !!existing?.canRead, canWrite: !!existing?.canWrite }; }) }); form.setFieldsValue({ ...grant, ...normalized, departmentScopes: grant.role === "admin" ? [] : normalized.departments }); }
