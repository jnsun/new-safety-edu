    ...
    <div className={`admin-workspace monthly-report-page${view === "mine" ? " is-baseline-mine" : ""}`}>
      ...
      {batchOpen && <section className="monthly-batch-page">
        ...
        {preflight.isLoading ? <Typography.Text>正在核对服务端数据…</Typography.Text> : preflight.isError ? <Alert type="error" message="核对失败" description={<Button onClick={() => void preflight.refetch()}>重试</Button>} /> : <>
          <Alert type={preflight.data?.ready ? "success" : "warning"} showIcon message={preflight.data?.ready ? "草稿已齐，可以整批提交" : "本次报送暂不可提交"} description={preflight.data?.reasons.join("；") || `应报 ${preflight.data?.expectedCount ?? 0} 个项目，已保存 ${preflight.data?.completedCount ?? 0} 个草稿。`} />
