...
export function TrainingPage() {
  ...
  const [form] = Form.useForm(); const [automationForm] = Form.useForm();
  const [open, setOpen] = useState(false); const [automationOpen, setAutomationOpen] = useState(false); const [editing, setEditing] = useState<Batch>();
  const [examRequired, setExamRequired] = useState(false); const [trainingType, setTrainingType] = useState<string>();
  ...
  const projectOnlyManager = !companyAdmin && !organizationManager;
  const departmentOptions = (organizations.data ?? []).filter(({ type }) => ["department", "business_entity"].includes(type)).map(({ id, name }) => ({ value: id, label: name }));

  const openCreate = () => {
    setExamRequired(false); setTrainingType(undefined); form.resetFields();
    form.setFieldsValue({ durationMin: 30, passScore: 80, maxAttempts: 3, examRequired: false, organizationIds: companyAdmin ? departmentOptions.map(({ value }) => value) : [] });
    setOpen(true);
  };
  ...
  return <div className="admin-workspace training-schedule-page">
    ...
    <Modal title="创建培训并下发" open={open} footer={null} onCancel={() => setOpen(false)}><Form form={form} layout="vertical" onFinish={async ({ examRequired: _examRequired, ...values }) => { try { if (!examRequired) delete values.paperId; if (values.dueAt) values.dueAt = new Date(String(values.dueAt)).toISOString(); else delete values.dueAt; await api("/api/training-batches", json("POST", values)); message.success("下发成功"); setOpen(false); await refresh(); } catch (error) { message.error(errorText(error)); } }}><Form.Item name="name" label="培训名称" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="type" label="培训类型" rules={[{ required: true }]}><Select options={types} onChange={(value) => { setTrainingType(value); const mandatory = ["three_level", "project_induction"].includes(value); setExamRequired(mandatory); form.setFieldValue("examRequired", mandatory); }} /></Form.Item><Form.Item name="templateId" label="模板" rules={[{ required: true }]}><Select options={(templates.data ?? []).filter((template) => !trainingType || template.type === trainingType).map(({ id, name }) => ({ value: id, label: name }))} /></Form.Item>{(trainingType === "project_induction" || projectOnlyManager) ? <Form.Item name="projectId" label="项目（下发给项目全体成员）" rules={[{ required: true }]}><Select options={(projects.data ?? []).filter(({ status }) => status === "active").map(({ id, name }) => ({ value: id, label: name }))} /></Form.Item> : companyAdmin ? <Form.Item name="organizationIds" label="选择部门（默认全部）" rules={[{ required: true, message: "至少选择一个部门" }]}><Select mode="multiple" options={departmentOptions} maxTagCount="responsive" /></Form.Item> : <Alert type="info" showIcon message="本次培训将下发给你授权组织的全体在用人员。" style={{ marginBottom: 16 }} />}<Form.Item name="dueAt" label="截止时间"><Input type="datetime-local" /></Form.Item><Form.Item name="examRequired" label="需要考试" valuePropName="checked"><Switch disabled={["three_level", "project_induction"].includes(trainingType ?? "")} onChange={setExamRequired} /></Form.Item>{examRequired && <><Form.Item name="paperId" label="试卷" rules={[{ required: true }]}><Select options={(papers.data ?? []).map(({ id, name }) => ({ value: id, label: name }))} /></Form.Item><Space><Form.Item name="durationMin" label="时长（分钟）"><InputNumber min={1} /></Form.Item><Form.Item name="passScore" label="合格分"><InputNumber min={0} max={100} /></Form.Item><Form.Item name="maxAttempts" label="最大次数"><InputNumber min={1} /></Form.Item></Space></>}<Button type="primary" htmlType="submit">确认下发</Button></Form></Modal>
