...
assert.match(app, /queryKey: \["wechat-web-widget-config"\][\s\S]*?refetchOnWindowFocus: false/);
assert.match(app, /queryKey: \["wechat-web-widget-config"\][\s\S]*?staleTime: 10 \* 60_000/);
assert.match(app, /wechatErrorCode = searchParams\.get\("wechat"\)/);
assert.match(app, /loginMode, setLoginMode\] = useState<"wechat" \| "password">\(wechatErrorCode \? "password" : "wechat"\)/);
...
assert.match(styles, /\.login-password-panel \{ justify-content: center; \}/);
assert.match(styles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.login-flip-inner \{ transition: none; \}/);
