...
## 2026-09 Figma 安全管理工作台层

安全管理后台页面以 Figma 文件 `FyvVcVfmf7ie1ubvjqDvwt` 的 `allpages (60:262)` 为当前视觉依据。该层作用于非 `.receivables-shell` 的安全管理页面；财务应收继续保留后文定义的高密度局部层，两个业务系统不共用权限判断。
...
- 左侧导航：桌面固定 `224px`
- 顶栏：`64px`
- 页面内容：1440px 视口使用 32px 水平内边距，1280px 收至 24px，1920px 放宽至 48px；页面级禁止横向滚动，宽表只在表格容器内部滚动。

页面头由标题、说明和右侧动作组成；白色卡片通过边框和轻环境阴影分层。蓝色只用于当前导航、链接和主动作。加载、空数据、筛选无结果、请求失败、无权限、业务冲突和未保存离开必须分别呈现，不能用同一个“暂无数据”代替。
