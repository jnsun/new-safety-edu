/* ===========================================================================
 * _kit.js — 安全生产培训教育平台 · 视觉规范的 Figma 绘图工具箱
 * 被各分区脚本用 `//!include ./_kit.js` 内联。
 * 所有函数均为同步：字体在 loadFonts() 里一次性预加载（约 15 个 style），
...
/**
 * 公司 Logo 矢量重绘。
 * 结构：青绿主环占主体，其余色相（黄绿 / 蓝 / 橙 / 灰）以扇段切入。
 * 分片之间留 3° 缝隙，避免小尺寸下糊成一团。
 * 注意：这是按结构重绘的矢量版本，正式资产到位后应替换。
 */
function logoGroup(size, o) {
  o = o || {};
  const mono = o.mono || null;          // 传色值即单色反白版
  const g = frame('Logo/' + size, { dir: 'NONE', w: size, h: size });
  g.fills = [];
  g.strokes = [];
  const rOut = size * 0.5;
  const rIn = size * 0.315;
  const segs = mono
    ? [[0, 360, mono]]
    : [
      [0, 250, C.brand],            // 主环青绿
      [253, 297, C.accent],         // 黄绿分片
      [300, 336, C.info],           // 蓝分片
      [339, 356, C.warning],        // 橙分片
      [359, 371, C.lineStrong],     // 灰分片（跨 0° 收口）
    ];
  for (let i = 0; i < segs.length; i++) {
    const [a1, a2, color] = segs[i];
    const closeAll = a2 - a1 >= 360;
    add(g, closeAll
      ? ringFullVector(size, rOut, rIn, color, 'ring·' + color)
      : ringSegVector(size, rOut, rIn, a1, a2, color, 'ring·' + color));
  }
  return g;
}
...
/** 小程序屏幕外壳：状态栏 + 内容 + 可选 TabBar。 */
function mpShell(name, o) {
  o = o || {};
  const ink = o.ink || C.ink;
  const outer = frame('▣ ' + name, { dir: 'VERTICAL', gap: 0, w: MP_W, h: MP_H, fill: o.fill || C.canvas, clips: true });
  outer.cornerRadius = 0;

  const sb = row('状态栏', { w: MP_W, h: 44, padX: 22, cross: 'CENTER', fill: o.statusFill != null ? o.statusFill : null });
  add(sb, text('9:41', { size: 12.5, weight: 700, color: ink }));
  add(sb, flex());
  add(sb, text('▮▮▮  ⌁  ▰', { size: 11, weight: 500, color: ink }));
  add(outer, sb, 'H');

  const body = col('内容区', { gap: 0, w: MP_W, fill: o.bodyFill != null ? o.bodyFill : null });
  add(outer, body, 'B');

  return { outer, sb, body, name };
}

/** 小程序自定义 TabBar（5 项）。 */
function mpTabBar(activeIdx) {
  const items = [
    ['待办', '✓'], ['记录', '☰'], ['闯关', '◈'], ['消息', '✉'], ['我的', '◉'],
  ];
  const bar = row('TabBar', { w: MP_W, h: 76, padTop: 8, fill: C.surface, stroke: C.line, sw: 1, cross: 'MIN' });
  bar.paddingBottom = 16;
  for (let i = 0; i < items.length; i++) {
    const on = i === activeIdx;
    const c = col('tab·' + items[i][0], { gap: 4, cross: 'CENTER', padX: 0, main: 'MIN' });
    add(c, text(items[i][1], { size: 17, weight: 500, color: on ? C.brand : C.muted }), 'H');
    c.children[0].textAlignHorizontal = 'CENTER';
    add(c, text(items[i][0], { size: 10.5, weight: on ? 700 : 500, color: on ? C.brandText : C.muted }), 'H');
...
/** Web 后台外壳：深墨绿侧栏 + 顶栏 + 内容区。 */
function webShell(name, o) {
  o = o || {};
  const outer = row('▣ ' + name, { gap: 0, w: WEB_W, h: WEB_H, fill: C.canvas, clips: true });

  // ---- 侧栏 224px ----
  const side = col('侧栏', { w: 224, fill: C.brandInk, padX: 0, padY: 0, gap: 0 });
  const brandRow = row('品牌', { w: 224, h: 64, padX: 20, gap: 10, cross: 'CENTER' });
  const lg = logoGroup(24, { mono: C.accent });
  add(brandRow, lg);
  const bt = col('品牌字', { gap: 1 });
  add(bt, text('安全培训', { size: 14, weight: 700, color: C.white }));
  add(bt, text('EDU PLATFORM', { size: 8.5, weight: 500, color: C.accent, ls: 0.6 }));
  add(brandRow, bt);
  add(side, brandRow, 'H');

  const menuWrap = col('菜单', { w: 224, gap: 2, padX: 12, padY: 10 });
  const MENU = o.menu || [
    ['◫', '工作台', 0], ['▤', '培训任务', 1], ['◈', '题库与试卷', 0], ['◉', '人员与组织', 0],
    ['▦', '培训计划', 0], ['❏', '证照管理', 0], ['▩', '统计报表', 0], ['⚙', '系统设置', 0],
  ];
  for (const [icon, label, on] of MENU) {
    const it = row('menu·' + label, {
      w: 200, h: 46, padX: 12, gap: 12, cross: 'CENTER', radius: 8,
      fill: on ? C.brand : null, fillOpacity: on ? 0.16 : null,
    });
    if (on) {
    ...
    add(it, text(icon, { size: 14, weight: 500, color: on ? C.accent : C.brandLine }), 'H');
    add(it, text(label, { size: 13.5, weight: on ? 700 : 400, color: on ? C.accent : '#CFE2DC' }), 'H');
    add(menuWrap, it, 'H');
  }
  add(side, menuWrap, 'H');
  add(side, flex(), 'H');

  const foot = row('侧栏脚', { w: 224, h: 60, padX: 18, gap: 10, cross: 'CENTER', fill: C.brandDeep });
  add(foot, dot(C.accent, 8));
  const fu = col('用户', { gap: 1 });
  add(fu, text(o.user || '孙金宁 · 公司管理员', { size: 12, weight: 600, color: C.white }));
  add(fu, text('安全生产培训平台', { size: 9.5, weight: 400, color: '#9FC4B9' }));
  add(foot, fu);
  add(side, foot, 'H');
  add(outer, side, 'V');
  ...
  const top = row('顶栏', { h: 64, padX: 28, cross: 'CENTER', fill: C.surface, stroke: C.line, sw: 1 });
  add(top, text(o.crumb || '工作台', { size: 16, weight: 700, color: C.ink }));
  add(top, flex());
  const srch = row('搜索', { w: 240, h: 36, padX: 12, gap: 8, cross: 'CENTER', radius: 8, fill: C.surfaceSoft, stroke: C.line, sw: 1 });
  add(srch, text('⌕', { size: 13, color: C.muted }));
  add(srch, text('搜索任务 / 人员', { size: 12.5, color: C.muted }));
  add(top, srch);
  add(top, box(16, 1));
  add(top, dot(C.accent, 32), null);
  add(main, top, 'H');

  const content = col('内容', { gap: 20, padX: 28, padY: 24, fill: C.canvas });
  add(main, content, 'B');
...
function webPageHead(title, desc, actions) {
  const r = row('页头', { gap: 16, cross: 'CENTER' });
  const l = col('页头左', { gap: 5 });
  add(l, text(title, { size: 22, weight: 700, color: C.ink }), 'H');
  if (desc) add(l, text(desc, { size: 12.5, weight: 400, color: C.muted }), 'H');
  add(r, l, 'H');
  if (actions) {
    const ra = row('页头动作', { gap: 10, cross: 'CENTER' });
    for (const a of actions) add(ra, a);
    add(r, ra);
  }
  return r;
}
